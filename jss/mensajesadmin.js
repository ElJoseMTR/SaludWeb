document.addEventListener('DOMContentLoaded', function() {
    ...
    if (user) {
        fetch(`https://api-base-de-datos.onrender.com/getmensajesadmin/${user}`)
                        ...
                        messageHtml += `
                            <div class="card">
                                <div class="card-header" id="heading-${adminId}">
                                    <h5 class="mb-0">
                                        <button class="bts" data-toggle="collapse" data-target="#collapse-${adminId}" aria-expanded="true" aria-controls="collapse-${adminId}">
                                            ${admin}
                                        </button>
                                    </h5>
                                </div>
...
function submitFeedback(response, feedback, messageId, asunto) {
    if (feedback === 'si') {
        ...
        fetch(`https://api-base-de-datos.onrender.com/deleterespuestas/${asunto}`, { method: 'DELETE' })
            .then(response => response.json())
            .then(data => {
                console.log(data);
                return fetch(`https://api-base-de-datos.onrender.com/deletemensajes/${asunto}`, { method: 'DELETE' });
            })
            .then(response => response.json())
            .then(data => {
                console.log(data);
