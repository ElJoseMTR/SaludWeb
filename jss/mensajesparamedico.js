document.addEventListener('DOMContentLoaded', function () {
    ...
    if (user) {
        fetch(`https://api-base-de-datos.onrender.com/getmensajesuserMedico/${user}`)
...
function submitResponse(event, admin, asunto) {
    ...
    fetch('https://api-base-de-datos.onrender.com/add_respuesta_medico', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ user: admin, usermedico: user, asunto: asunto, mensaje: responseText })
    })
    .then(response => response.json())
    .then(data => {
